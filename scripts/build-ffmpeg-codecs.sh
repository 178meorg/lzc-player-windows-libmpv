...
build_cmake() {
    ...
    if [[ -n "${patch_file}" ]]; then
        ...
        elif git -C "${source}" apply --ignore-whitespace --check "${patch_file}"; then
...
build_x264() {
    local source
    source="$(clone_git x264 https://code.videolan.org/videolan/x264.git stable)"
    cd "${source}"
    make distclean >/dev/null 2>&1 || true
    # MSYS2 MINGW64 uses native gcc/binutils names, not cross-prefixed tools.
    # x264 enables shared builds explicitly, so --disable-shared is unsupported.
    ./configure \
        --prefix="${PREFIX}" \
        --libdir="${PREFIX}/lib" \
...
build_autotools speexdsp https://github.com/xiph/speexdsp.git "SpeexDSP-${SPEEX_VERSION}" ""
build_autotools speex-codec https://github.com/xiph/speex.git "Speex-${SPEEX_VERSION}" "--disable-binaries"
build_cmake soxr https://git.code.sf.net/p/soxr/code "${LIBSOXR_VERSION}" . \
    "-DBUILD_TESTS=OFF -DBUILD_EXAMPLES=OFF -DWITH_OPENMP=OFF -DCMAKE_POLICY_VERSION_MINIMUM=3.5"
...
build_meson dav1d https://code.videolan.org/videolan/dav1d "${DAV1D_VERSION}" "-Denable_tests=false -Denable_tools=false"
build_cmake aom https://aomedia.googlesource.com/aom "v${AOM_VERSION}" . \
    "-DAOM_BUILD_APPS=OFF -DENABLE_TESTS=OFF -DENABLE_TOOLS=OFF -DENABLE_EXAMPLES=OFF -DENABLE_DOCS=OFF -DENABLE_NASM=ON" \
    "${ROOT_DIR}/patches/aom-nasm-3-help.patch"
...
build_x264
build_cmake x265 https://bitbucket.org/multicoreware/x265_git "${X265_VERSION}" source \
    "-DENABLE_CLI=OFF -DCMAKE_POLICY_VERSION_MINIMUM=3.5" \
    "${ROOT_DIR}/patches/x265-cmake-4-policies.patch"

# FFmpeg checks LAME and SoXR by linking; their builds do not provide .pc files here.
for file in \
    "${PREFIX}/lib/libmp3lame.a" \
    "${PREFIX}/include/lame/lame.h" \
    "${PREFIX}/lib/libsoxr.a" \
    "${PREFIX}/include/soxr.h"; do
    test -f "${file}" || {
        echo "Missing codec dependency file: ${file}" >&2
        exit 1
    }
done

for library in opus ogg vorbis speexdsp speex vpx dav1d aom SvtAv1Enc libwebp x264 x265; do
